 ...
 *    - the post-plora_gateway.cpp~:1516:    		printf("^$Check for downlink requests %s\n", time_buffer);
rocessing stage looks for this sequence to dissect the data, according to the format adopted by the sender 
...
unsigned long interDownlinkCheckTime=100L;					//kan 5000L
...
#else
uint8_t loraChannelIndex=14;      //avi was 6
#endif
...
#ifdef RECEIVE_ALL// THE MAIN PACKET RECEPTION LOOP
...
///////////////////////////////////////////////////////////////////
         ...
         elselora_gateway.cpp~:1516:    		printf("^$Check for downlink requests %s\n", time_buffer);
...
#ifdef LORA_LAS
              loraLAS.setSIFS(loraMode);lora_gateway.cpp~:1516:    		printf("^$Check for downlink requests %s\n", time_buffer);
    	  ...
    	  printf("^$Process downlink requests %d: %s\n", dl_line_index, json_entry[dl_line_index]);
    		...
    		if (document["status"]=="send_request" && document["dst"].IsInt()) {
    			...
    			if (!CarrierSense(true)) {
    				...
    				if (document["MIC"].IsString() && document["MIC"]!="") {
    					...
    					downlink_message[l]=(uint8_t)xtoi(&(document["MIC"].GetString()[0]));
    					downlink_message[l+1]=(uint8_t)xtoi(&(document["MIC"].GetString()[1]));
    					downlink_message[l+2]=(uint8_t)xtoi(&(document["MIC"].GetString()[2]));
    					downlink_message[l+3]=(uint8_t)xtoi(&(document["MIC"].GetString()[3]));
...
#else    				
    				// here we sent the downlink packet
    				//printf("--------------------------------------------------------here we send the message to the end node %d\n\r",document["data"].GetStringLength());
					//uint8_t* packet = ((uint8_t*)base64_decode(document["data"].GetString() ).c_str());		
					//for(int i = 0 ; i < document["data"].GetStringLength() ;  i++) {
					//	printf("%d  ",(int)((uint8_t*)document["data"].GetString())[i]);
					//}	
					//printf("\n");
    				e = sx1272.sendPacketTimeout(document["dst"].GetInt(), (uint8_t*)document["data"].GetString(), document["data"].GetStringLength(), 10000);    //avi come back    pleaaaaaaaase
    				//printf("our document												:   %s      \n\n  ",(uint8_t*)document["data"].GetString() );
#endif    				
