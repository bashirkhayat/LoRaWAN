...
SX1272::SX1272()
{
    ...
    _retries = 0;
    // added by C. Pham
    _defaultSyncWord=0x34;  ////avi was 12
    _rawFormat=true;				////avi was false
    _extendedIFS=false;
    _RSSIonSend=true;
    // disabled by default
    ...
    if( p_received )
    {
		...
		if (_reception == CORRECT_PACKET) {
			...
			else
			{
				...
				for(unsigned int i = 0; i < _payloadlength; i++)
				{
					printf("%x", packet_received.data[i]);		//avi was %c
				}
				printf("\n");
				//printf("Retry number: ");
...
uint8_t SX1272::setPayload(char *payload)
{
    ...
    state = 1;
    //length16 = (uint16_t)strlen(payload);   //avi commented this
...
*/       //Bashir commented the lines above
        ...
        for(unsigned int i = 0; i < _payloadlength; i++)
        {
            printf("%x", packet_sent.data[i]);		// Printing payload
        }
        printf("\n");
        //printf("Retry number: ");
...
uint8_t SX1272::setPacket(uint8_t dest, uint8_t *payload)
{
    ...
    if (_limitToA) {
        ...
        if (!_rawFormat) //jeries commented this
            length16 = length16 + OFFSET_PAYLOADLENGTH;
...
uint8_t SX1272::sendPacketTimeout(uint8_t dest, uint8_t *payload, uint16_t length16, uint16_t wait)
{
	...
	std::string command = "python decode64.py "+std::string((char*)payload);
	uint8_t packet[length16];	
	FILE* in = popen(command.c_str(),"r");

	int i = 0;
	while(fscanf(in,"%d",&packet[i]) == 1) {
		i++;
	}	
	
	fclose(in);	
    state = truncPayload(i); //was length16
    ...
    {
			_payloadlength = i;
        state_f = setPacket(dest, packet);	// Setting a packet with 'dest' destination
        //state_f = setPacket(dest, payload);	// Setting a packet with 'dest' destination
    }
    else
    {
